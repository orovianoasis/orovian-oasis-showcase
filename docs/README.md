# Documentation

The repository root stays intentionally clean. Guides, references, deployment notes, and generated audit records live here.

## Getting started

- [Quick Start](getting-started/QUICK_START.md)
- [Upload Instructions](getting-started/UPLOAD_THIS_FOLDER.txt)

## Editing and content

- [Editing Guide](editing/EDITING_GUIDE.md)
- [Content Workflow](editing/CONTENT_WORKFLOW.md)
- [Folder Naming Guide](editing/FOLDER_NAMING_GUIDE.md)
- [Mobile Design Guide](editing/MOBILE_DESIGN_GUIDE.md)
- [Contact Setup](editing/CONTACT_SETUP.md)

## Deployment

- [Custom Domain Setup](deployment/CUSTOM_DOMAIN_SETUP.md)

## Reference

- [Repository Map](reference/REPOSITORY_MAP.md)
- [Project Field Reference](reference/PROJECT_FIELD_REFERENCE.md)
- [Release Checklist](reference/RELEASE_CHECKLIST.md)

## Internal audit records

- `internal/VALIDATION_REPORT.txt`
- `internal/STRUCTURE_MANIFEST.txt`
- `internal/SHA256SUMS.txt`

