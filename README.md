...
Add `property-details.json` to the same folder for one-file control of the public title, description, category, status, square footage, bedrooms, bathrooms, garage, stories, price, checkout link, button behavior, and optional walkthrough camera/layer settings. GitHub Actions automatically converts the DAE into a browser GLB and connects the permanent Orovian Oasis walkthrough controls. No Chief cloud account or ChatGPT conversion is required.

Existing folders that already contain a custom walkthrough HTML and GLB remain supported unchanged. Original long filenames and underscores are accepted; public URLs are converted to hyphens automatically.
...
Start with the [Chief Architect DAE workflow](docs/editing/CHIEF_ARCHITECT_DAE_WORKFLOW.md) or the [one-folder upload guide](docs/editing/ONE_FOLDER_PROJECT_UPLOAD.md).
...
- [Chief Architect DAE walkthrough workflow](docs/editing/CHIEF_ARCHITECT_DAE_WORKFLOW.md)
